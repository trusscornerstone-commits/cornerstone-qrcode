  ...
  async function startCamera(userInitiated=false) {
    ...
    function startScanLoop() {
      ...
      const ctx = canvas.getContext('2d');
       ...
       // Append para debug (remova depois)
      ...
      const loop = () => {
        ...
        document.body.appendChild(canvas); // Apenas para depuração temporária

        ctx.drawImage(videoEl, 0, 0, canvas.width, canvas.height);

        console.log('Vídeo size:', canvas.width, canvas.height);

        const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        const code = jsQR(imageData.data, canvas.width, canvas.height);

        console.log('jsQR output:', code);
